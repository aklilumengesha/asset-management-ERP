...
export function CategoryDialog({ open, onOpenChange, category, onSave }: CategoryDialogProps) {
  ...
  const ifrsClassOptions = ifrsClassifications.map(c => ({
    value: c.class,
    label: `${c.name} (${c.class})`
  }));

  const filteredIfrsCategories = ifrsCategories.filter(cat => 
    !formData.ifrsClassification || cat.class === formData.ifrsClassification.class
  );

  const ifrsCategoryOptions = filteredIfrsCategories.map(c => ({
    value: c.code,
    label: `${c.name} (${c.code})`
  }));

  const taxCategoryOptions = taxCategories.map(c => ({
    value: c.code,
    label: `${c.name} (${c.code})`
  }));

  const handleIFRSClassChange = (classCode: string) => {
    const classification = ifrsClassifications.find(c => c.class === classCode);
    setFormData(prev => ({
      ...prev,
  ...
  const handleIFRSCategoryChange = (categoryCode: string) => {
    const category = ifrsCategories.find(c => c.code === categoryCode);
    setFormData(prev => ({
      ...prev,
  ...
  const handleTaxCategoryChange = (categoryCode: string) => {
    const category = taxCategories.find(c => c.code === categoryCode);
    setFormData(prev => ({
      ...prev,
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-w-2xl">
          ...
          <div className="flex items-center justify-center p-8">
            <p className="text-muted-foreground">Loading depreciation categories...</p>
          </div>
        </DialogContent>
      </Dialog>
    );
